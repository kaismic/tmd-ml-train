...
class TMDetection:
    ...
    def __get_full_dataset_for_classification(self, sensors_set):
        ...
        # Fill NaN with column mean across the full set
        for col in features:
            col_mean = full_df[col].mean()
            full_df[col] = full_df[col].fillna(col_mean)
        all_features = full_df[features].values.astype(np.float32)
        all_classes = [self.classes2number[c] for c in full_df['target'].values]
        return features, all_features, all_classes
 

    def __save_onnx(self, classifier, n_features, sensors_set, model_name):
        ...
        out_path = os.path.join(models_dir, str(sensors_set) + '_' + model_name + '.onnx')
        with open(out_path, 'wb') as f:
            f.write(onnx_model.SerializeToString())
        print("ONNX model saved to: " + out_path)

    # Train a Decision Tree on the full dataset and export as .onnx
    def create_decision_tree_model(self, sensors_set):
        print("CREATING DECISION TREE ONNX MODEL.....")
        print("CLASSIFICATION BASED ON THESE SENSORS: ", self.dataset.get_remained_sensors(sensors_set))
        features, all_features, all_classes = self.__get_full_dataset_for_classification(sensors_set)
        print("NUMBER OF FEATURES: ", len(features))
        classifier = tree.DecisionTreeClassifier()
        classifier.fit(all_features, all_classes)
        self.__save_onnx(classifier, len(features), sensors_set, 'decision_tree')
        print("END DECISION TREE ONNX MODEL")

    # Train a Random Forest on the full dataset and export as .onnx
    def create_random_forest_model(self, sensors_set):
    ...
    def create_neural_network_model(self, sensors_set):
        ...
        classifier.fit(all_features, all_classes)
        self.__save_onnx(classifier, len(features), sensors_set, 'neural_network')
    ...
    def create_support_vector_machine_model(self, sensors_set):
        ...
        classifier = Pipeline([
            ('scaler', StandardScaler()),
            ('svc', SVC(
                C=const.PAR_SVM_C[sensors_set],
                gamma=const.PAR_SVM_GAMMA[sensors_set],
                verbose=False
            ))
        ])
        classifier.fit(all_features, all_classes)
        self.__save_onnx(classifier, len(features), sensors_set, 'support_vector_machine')
...
#	detection.create_decision_tree_model(1)
#	detection.create_decision_tree_model(2)
#	detection.create_decision_tree_model(3)

#	detection.create_random_forest_model(1)
#	detection.create_random_forest_model(2)
	detection.create_random_forest_model(3)

#	detection.create_neural_network_model(1)
#	detection.create_neural_network_model(2)
#	detection.create_neural_network_model(3)

#	detection.create_support_vector_machine_model(1)
#	detection.create_support_vector_machine_model(2)
#	detection.create_support_vector_machine_model(3)

...
class TMDataset:
    ...
    def transform_raw_data(self):
        ...
        for file in filenames:
            if file.endswith(".csv"):
                with open(os.path.join(dir_src, file), encoding='utf-8', errors='replace') as current_file:
                    with open(os.path.join(dir_dst, file), "w", encoding='utf-8') as file_result:
                        for line in current_file:
                            line_data = line.split(",")
                            endLine = ",".join(line_data[2:])
                            current_time = line_data[0]
    ...
    def __fill_data_structure(self):
        ...
        for file in filenames:
            if file.endswith(".csv"):
                ...
                f = open(os.path.join(dir_src, file), encoding='utf-8', errors='replace')
    ...
    def create_header_files(self):
        ...
        for file in filenames:
            if file.endswith(".csv"):
                ...
                with open(full_current_file_path, encoding='utf-8', errors='replace') as current_file:
                    full_current_file_path = os.path.join(dir_dst, file)
                    with open(full_current_file_path, "w", encoding='utf-8') as file_header:
        ...
        with open(result_file_path, 'w') as result_file:
            ...
            for file in filenames:
                if file.endswith(".csv"):
                    ...
                    with open(current_file_path, encoding='utf-8', errors='replace') as current_file:  # tm file
                        i = 0
                        for line in current_file:
    ...
    def create_balanced_dataset(self, sintetic = False):
        ...
        target_df['percent'] = target_df.groupby(level=0)['target'].transform(lambda x: 100 * x / x.sum())
        ...
        for index, row in target_df.iterrows():
            target, user = tuple(index)  # type: ignore[arg-type]
            current_df = df.loc[(df['user'] == user) & (df['target'] == target)]
